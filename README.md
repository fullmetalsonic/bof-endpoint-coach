...
![취련 코치 v0.6 G6 대시보드](docs/manual/screenshots/v0.6.0/03-dashboard-g6.png)
...
- **28개 최신 화면이 포함된 초상세 설명서**와 한영 요약 설명서

![저장·복구·보고서](docs/manual/screenshots/v0.6.0/01-storage-recovery.png)

### 바로 사용하기

1. [Releases](https://github.com/fullmetalsonic/bof-endpoint-coach/releases/latest)에서 `BOF_Endpoint_Coach_v0.6.0.html`을 받습니다.
...
7. 교대·브라우저 정리·PC 이동·계수 변경 전 `분석 · 리포트 > 전체 데이터 JSON으로 저장`을 실행합니다.

요약 절차는 [한영 사용 설명서](docs/user-guide.md), 모든 명칭·단위·입력 예·오류조건은 [초상세 처음사용자 설명서](docs/manual/취련코치_초상세_처음사용자_설명서_v0.6.0.md), 검증 결과는 [v0.6.0 최종 감사](docs/audits/v0.6.0_최종_기능_UI_UX_회귀감사_2026-08-23.md)에 있습니다.
...
`npm run check`는 lint, 126개 단위·통합 시험, 일반 빌드, 단일 HTML 빌드, Sites worker 시험을 실행합니다. Chromium E2E는 공정입력, 다중차지, 정정·출강, JSON 전체복원·취소, 오프라인 단일파일, 다중 창 충돌, 반응형과 문서 렌더링을 확인합니다. Chrome·Edge 채널과 실제 회사 PC 정책은 별도 환경 검증이 필요합니다.
...
BOF Endpoint Coach v0.6.0 is a single-file, offline desktop assistant. Operators manually enter heat, process, sample, and confirmed endpoint data. The tool starts with transparent public-literature scenarios and preserves plant/equipment-specific residual evidence so reviewed endpoint estimates can improve over time.
...
1. Download `BOF_Endpoint_Coach_v0.6.0.html` from the [latest release](https://github.com/fullmetalsonic/bof-endpoint-coach/releases/latest).
...
7. Export the full verified JSON before handover, browser cleanup, PC transfer, or coefficient changes.
