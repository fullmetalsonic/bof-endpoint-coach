...
- 브라우저 화면: 1920 × 1080, 확대 100%
- 상태: 합성 DEMO G6 차지, 한국어
- 화면: 대시보드와 `기준 정보 → 계산 · 보정 계수`

## 확인·수정 결과

- 대시보드는 상단 차지 요약, G0~G8, 6개 품질행, 최근 분석표, Data Ledger, 하단 6개 입력 버튼을 한 화면에서 유지했다.
- C·온도 문헌 시나리오 범위와 문헌 기본 상태를 추가해도 기존 전문형 정보 밀도가 무너지지 않았다.
- 온도 실측점과 예상점이 가까울 때 숫자 라벨이 겹치는 문제를 발견했다. 예상 라벨을 막대 위, 실측 라벨을 아래로 분리하고 재확인했다.
- 계수 설정은 문헌 원본, 현장 수정값, 실제 적용값, 근거 ID를 가로 표로 분리했다. 내부 세로 스크롤로 19개 항목을 관리한다.
- 수정값 입력 즉시 `사용자 수정 · 미승인`으로 전환되고 실제 적용값이 바뀌었다.
- 현장 승인 선택 뒤 승인자·근거가 없으면 저장을 막고, 두 값이 채워지면 저장이 활성화됐다.
- 수정값 전체 지우기 뒤 문헌 기본 상태와 빈 수정칸으로 복귀했다.
- 브라우저 오류·경고 로그는 0건이었다.
