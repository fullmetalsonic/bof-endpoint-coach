    ...
    <section className="analysis-table panel">
      ...
      <div className="table-scroll">
        <table>
          ...
          <tbody>{samples.map(({ sample, analysis }) => <tr key={sample.id}><td>{formatTime(sample.sampledAt)}</td><td>{sample.stage}</td><td>{sample.id}</td><td>{format(analysis?.values?.C)}</td><td>{format(analysis?.values?.Si, 2)}</td><td>{format(analysis?.values?.Mn, 2)}</td><td>{format(analysis?.values?.P)}</td><td>{format(analysis?.values?.S)}</td><td>{format(analysis?.values?.temperature, 0)}</td><td>{analysis?.method ?? "Pending"}</td><td><div className="sample-actions">{analysis ? <><button type="button" onClick={() => onCorrection({ mode: "correct", targetKind: "analysis", targetId: analysis.id })}>{ko ? "수정" : "Correct"}</button><button type="button" className="danger-link" onClick={() => onCorrection({ mode: "void", targetKind: "analysis", targetId: analysis.id })}>{ko ? "무효" : "Void"}</button></> : <button type="button" onClick={onOpenTimeline}>{ko ? "이력" : "Timeline"}</button>}</div></td></tr>)}</tbody>
        </table>
      </div>
    </section>
  );
