...
import { createDissolvedOxygenRecord } from "../domain/measurements/dissolvedOxygen.js";
...
const guidance = {
  ...
  analysis: { ko: "하나 이상의 분석값이 필요하며, 종점 검토에는 C와 온도를 함께 입력하는 것이 중요합니다. 용존산소는 값이 있을 때만 선택 입력합니다.", en: "At least one result is required; endpoint review should include both C and temperature. Dissolved oxygen is optional when available." },
...
function eventHelp(locale) {
  ...
  analysis: [{ term: "% / wt% / ppm", ko: "성분 농도 단위입니다. 1% = 10,000 ppm이며 내부 계산은 %로 환산합니다.", en: "Chemistry concentration units. 1% = 10,000 ppm; calculations normalize to percent." }, { term: locale === "ko" ? "샘플 시점 누적 산소" : "Oxygen at sample", ko: "분석값이 아니라 샘플을 채취했던 시각까지 공급된 산소량입니다. 샘플 보정 계산의 기준점입니다.", en: "Not an analysis result; this is the oxygen supplied by the sampling time and anchors the correction calculation." }, { term: locale === "ko" ? "용존산소 [O]" : "Dissolved oxygen [O]", ko: "용강 안의 산소 농도(ppm)입니다. 선택 기록이며 현재 계산에는 사용하지 않습니다.", en: "Oxygen concentration in the steel bath (ppm). It is an optional record and is not used by the current calculation." }],
...
export function EventModal({ action, heat, settings, locale, t, onClose, onSave }) {
  ...
  const defaults = useMemo(() => ({
    ...
    dissolvedOxygenPpm: "", dissolvedOxygenSource: "", dissolvedOxygenNote: "",
  ...
  const payload = {
    ...
    values,
    dissolvedOxygen: action === "analysis" ? createDissolvedOxygenRecord({ valuePpm: form.dissolvedOxygenPpm, source: form.dissolvedOxygenSource, note: form.dissolvedOxygenNote }) : undefined,
      ...
      <form ref={dialogRef} tabIndex="-1" className="event-modal" onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="event-modal-title">
        ...
        <div className="form-grid">
          ...
          {action === "analysis" && <>
            ...
            <OptionalDissolvedOxygenSection locale={locale} valuePpm={form.dissolvedOxygenPpm} source={form.dissolvedOxygenSource} note={form.dissolvedOxygenNote} onChange={setDissolvedOxygen} />
          </>}
          {action === "checkpoint" && <>
