...
import { assumptionLabel, modelLabel, objectiveLabel, reasonLabel, timingModeLabel } from "./additionText.js";
...
function AdditionComparisonDialog({ heat, settings, coach, locale, canWrite, onClose, onSavePlan, onDecision }) {
  ...
  const baseVersion = `${heat.id}:${latestProposal?.id ?? coach.calculatedAt}`;
  ...
  const valid = Number(form.amount) > 0 && (form.operationType === "oxygen" || Boolean(form.materialCode));
  ...
  async function copyCoach() {
    if (!recommendation || !canWrite || busy) return;
    setBusy(true);
    await onSavePlan({ operationType: recommendation.operationType, materialCode: recommendation.materialCode, amount: recommendation.amount.midpoint, timingMode: recommendation.timing?.startAt ? "local_time" : "now", plannedAt: recommendation.timing?.startAt ?? new Date().toISOString(), note: ko ? "코치 참고안을 계획 초안으로 복사" : "Copied from coach reference" });
    if (latestProposal?.id) onDecision(latestProposal.id, "copy_coach_to_plan");
    setBusy(false);
  }
  ...
  return <div className="modal-backdrop" role="presentation"><div ref={dialogRef} tabIndex="-1" className="addition-drawer" role="dialog" aria-modal="true" aria-labelledby="addition-comparison-title">
    ...
    <form className="addition-plan-form" onSubmit={savePlan}><div className="panel-title"><h3>{ko ? "내 계획 입력(선택)" : "Operator plan (optional)"}</h3><span>{ko ? "계획만 저장 · 실제값 아님" : "Plan only · not actual"}</span></div>
      ...
      <div className="addition-plan-fields"><label><span>{ko ? "조작" : "Action"}</span><select value={form.operationType} onChange={(event) => set("operationType", event.target.value)}><option value="material">{ko ? "재료 투입" : "Material"}</option><option value="oxygen">{ko ? "추가 산소" : "Additional oxygen"}</option></select></label>{form.operationType === "material" && <label><span>{ko ? "재료" : "Material"}</span><select value={form.materialCode} onChange={(event) => set("materialCode", event.target.value)}>{materialOptions.map((material) => <option key={material.code} value={material.code}>{material.code} · {ko ? material.nameKo : material.nameEn}</option>)}</select></label>}<label><span>{ko ? "예상량" : "Planned amount"}</span><div className="input-with-unit"><input type="number" min="0" step="0.001" value={form.amount} onChange={(event) => set("amount", event.target.value)} /><span>{form.operationType === "oxygen" ? "Nm³" : "kg"}</span></div></label><label><span>{ko ? "예정 기준" : "Timing basis"}</span><select value={form.timingMode} onChange={(event) => set("timingMode", event.target.value)}><option value="now">{ko ? "지금" : "Now"}</option><option value="local_time">{ko ? "로컬 시각" : "Local time"}</option><option value="elapsed">{ko ? "취련 경과" : "Elapsed time"}</option><option value="oxygen">{ko ? "누적 산소" : "Cumulative oxygen"}</option></select></label>{form.timingMode === "local_time" && <label><span>{ko ? "예정 시각" : "Planned time"}</span><input type="datetime-local" step="1" value={form.plannedAt} onChange={(event) => set("plannedAt", event.target.value)} /></label>}{form.timingMode === "elapsed" && <label><span>{ko ? "취련 경과" : "Elapsed"} (min)</span><input type="number" min="0" step="0.1" value={form.elapsedMinutes} onChange={(event) => set("elapsedMinutes", event.target.value)} /></label>}{form.timingMode === "oxygen" && <label><span>{ko ? "누적 산소" : "Cumulative oxygen"} (Nm³)</span><input type="number" min="0" step="1" value={form.cumulativeOxygenNm3} onChange={(event) => set("cumulativeOxygenNm3", event.target.value)} /></label>}</div>
      <div className="addition-plan-actions"><button type="submit" className="secondary-button" disabled={!valid || !canWrite || busy}>{ko ? "내 계획 저장" : "Save operator plan"}</button><button type="button" className="primary-button" disabled={!recommendation || !canWrite || busy} onClick={copyCoach}>{ko ? "코치안을 계획으로 복사" : "Copy coach to plan"}</button></div>
    </form>
    <div className="modal-actions"><button type="button" className="secondary" disabled={!latestProposal?.id || !canWrite} onClick={() => onDecision(latestProposal.id, "keep_operator_plan")}>{ko ? "내 계획 유지" : "Keep operator plan"}</button><button type="button" className="secondary" disabled={!latestProposal?.id || !canWrite} onClick={() => { onDecision(latestProposal.id, "defer_until_sample"); onClose(); }}>{ko ? "샘플 후 다시 보기" : "Review after sample"}</button><button type="button" className="primary" onClick={onClose}>{ko ? "닫기" : "Close"}</button></div>
  </div></div>;
}
