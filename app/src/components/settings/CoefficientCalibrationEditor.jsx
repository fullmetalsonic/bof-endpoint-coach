...
export function CoefficientCalibrationEditor({ profile, updateProfile, locale, candidate }) {
  ...
  return (
    <section className="coefficient-calibration" aria-labelledby="calibration-offset-title">
      <div className="subsection-heading">
        ...
        <span className="coefficient-version-id">{profile.versionId ?? "–"}</span>
      </div>
      ...
      <div className="calibration-offset-grid">{OFFSET_FIELDS.map(([key, ko, unit, step]) => <label key={key}><span>{locale === "ko" ? ko : key === "temperature" ? "Temperature" : key} <small>{unit}</small></span><input aria-label={`calibration-${key}`} type="number" step={step} value={profile.calibrationOffsets?.[key] ?? 0} onChange={(event) => updateOffset(key, event.target.value)} /></label>)}</div>
