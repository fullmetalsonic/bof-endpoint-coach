...
export function MaterialsEditor({ draft, setDraft, locale, t, usedMaterialCodes }) {
  ...
  return <>
    ...
    <div className="settings-form-grid identity-grid">
      ...
      <label><span>{t("category")}</span><select value={material.category ?? "other"} onChange={(event) => update("category", event.target.value)}><option value="flux">Flux</option><option value="coolant">Coolant / Ore</option><option value="alloy">Alloy</option><option value="carburizer">Carburizer</option><option value="scrap">Scrap</option><option value="other">Other</option></select></label>
