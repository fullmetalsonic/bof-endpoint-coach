...
export function Dashboard({ state, heat, locale, t, selectHeat, saveStatus, onAction, onAdvance, onEditInitial, onNewHeat, onOpenTimeline, onOpenCorrection, onRollback }) {
    ...
    <>
      ...
      <div className="dashboard-shell" data-testid="dashboard-screen">
        ...
        <main className="dashboard-main">
          <div className="dashboard-title-row"><h1>{heat.stage} {locale === "ko" ? heat.stageLabelKo : heat.stageLabelEn} {t("summary")}</h1><div className="dashboard-title-actions"><span>{locale === "ko" ? "입력 → 확인 → 단계 전환" : "Enter → verify → advance"}</span>{canRollbackLastStage(heat) && <button type="button" className="stage-undo-button" onClick={onRollback}>{locale === "ko" ? "마지막 단계 전환 취소" : "Undo last stage"}</button>}</div></div>
          <WorkflowPanel heat={heat} locale={locale} rows={rows} t={t} onAction={onAction} onAdvance={onAdvance} onEditInitial={onEditInitial} />
      ...
      <ActionBar t={t} locale={locale} onAction={onAction} availability={getActionAvailability(heat)} recommendedAction={workflow.recommendedAction} />
    </>
  );
}
