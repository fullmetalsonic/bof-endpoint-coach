...
export function Dashboard({ state, heat, locale, t, selectHeat, saveStatus, storageMeta, recoveryPointCount = 0, canWrite = true, onAction, onAdvance, onEditInitial, onNewHeat, onOpenTimeline, onOpenCorrection, onRollback, onRefreshAddition, onSaveAdditionPlan, onAdditionDecision, onSetAdditionHidden }) {
  ...
  const explanations = predictionExplanations(heat, rows, calculation, state.trainingRuns);
  ...
  return (
    <>
      ...
      <div className="dashboard-shell" data-testid="dashboard-screen">
        ...
        <DataLedger heat={heat} calculation={calculation} rows={rows} saveStatus={saveStatus} storageMeta={storageMeta} recoveryPointCount={recoveryPointCount} state={state} t={t} locale={locale} />
      </div>
